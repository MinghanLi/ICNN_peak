...
class MuraDataset(data.Dataset):
    _patient_re = re.compile(r'patient(\d+)')  # 正则表达式 \d 数字
    _study_re = re.compile(r'study(\d+)')
    _image_re = re.compile(r'image(\d+)')  # 匹配前一个字母一次或者无限次
    _study_type_re = re.compile(r'XR_(\w+)')  # 正则表达 \w 字母
    ...
    def __getitem__(self, idx):
        img_filename = join(self.imgs[idx]).strip(';')  # strip 删除首尾字符';'
        # print(img_filename)
