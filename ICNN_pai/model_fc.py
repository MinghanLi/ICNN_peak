...
class Model(nn.Module):
    def __init__(self, args=None):
        ...
        self.addconv1 = nn.Conv2d(M, M, kernel_size=3, stride=1, padding=1)
        self.addconv2 = nn.Conv2d(M, M, kernel_size=14, stride=1, padding=0)
        self.classification = nn.Linear(M, args.num_classes)
        self.out_size = 14
    ...
    def forward(self, x, train=True):
        ...
        # x = F.max_pool2d(x2, kernel_size=3, stride=1, padding=1)
        x = self.addconv2(x2).squeeze()
        x = self.classification(x).view(x.size(0), -1)

        # compute local loss:
        loss_1 = self.compute_local_loss(x1)
        loss_2 = self.compute_local_loss(x2)

        return x, x2, loss_1, loss_2
