...
def kappa(y_pred, y_GT):
    ...
    pred_soft = (F.softmax(y_pred, dim=1).t()[0] < 0.8).cpu().numpy().tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained', type=str, default='/data/oss_bucket/suqi/model/9vgg16ICNN_MURA.pth.tar')
    ...
    print('=>ICNN_peak, using net:', args.net, 'batchsize=', args.batch_size, 'learning_rate=', args.lr,
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (imgs, labels, meta) in enumerate(pbar):
            ...
            loss = loss + lambda_mask * (loss_1.sum() + loss_2.sum()) + lambda_peak * loss_peak
            losses += loss
        ...
        acc_kappa = classification.cohen_kappa_score(y_pred_all, y_actual_all)
        FN, FP = false_loss(y_actual_all, y_pred_all)
        print("Epoch%d Test kappa:%.4f, acc:%.4f, loss:%.4f, FN:%.4f, FP:%.4f" % (epoch, acc_kappa, acc, losses, FN, FP))
        rank = torch.distributed.get_rank()
        save_path = '/data/oss_bucket/suqi/train_models/ICNN_peak/vgg16_32_4_small/'
        if rank == 0:
            torch.save(model.state_dict(), save_path + str(epoch) + args.net + 'ICNN_peak_Luna16' + '.pth.tar')
        model.train()
